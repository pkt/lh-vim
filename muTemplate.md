...
µTemplate is a template-files loader for Vim. Once loaded, templates are interpreted and expanded according to a flexible syntax.
  ...
  * [Features](muTemplate#Features.md)
  * [Shipped templates](muTemplate#Shipped_templates.md)
    * [Vim](muTemplate#Vim.md)
    * [C & C++](muTemplate#C_&_C++.md)
    * [LaTeX](muTemplate#LaTeX.md)
    * [XSLT](muTemplate#XSLT.md)
    * [Licenses](muTemplate#Licenses.md)
    * [Other filetypes](muTemplate#Other_filetypes.md)
  * [Examples](muTemplate#Examples.md)
    * [C-`if` snippet](muTemplate#C-_if_snippet.md)
    * [C-`case` snippet](muTemplate#C-_case_snippet.md)
    * [Interactive template-file: C++ Class Template](muTemplate#Interactive_template-file:_C++_Class_Template.md)
    * [Completely useless recursive example](muTemplate#Completely_useless_recursive_example.md)
  * [Installation](muTemplate#Installation.md)
  * [Credits](muTemplate#Credits.md)
  * [See also](muTemplate#See_also.md)
...
## Features ##
  ...
  * Fully integrated with my [placeholders-system](lhBrackets.md) ;
  * Supports re-indentation (if desired) ;
  * Works well with vim folding ;
  * I18n friendly ;
  * The expansion happens after any _[local vimrcs](https://github.com/LucHermitte/local_vimrc)_ present are loaded -- in order to set project-specific variables before the expansion is done.
...
NB: there are no µTemplate snippets for `:if`, `:try`, _etc._ because they are maintained independently thanks to [my bracketing system](lhBrackets.md) in `vim_snippets.vim` (todo: add link).

### C & C++ ###
Most of my C and C++ template-files are shipped with [lh-cpp](lhCpp.md). However a few ones are still shipped with µTemplate:
  * On creation of a new C or C++ file, the file is filled according to the kind of file detected:
    ...
    * _source files_ first compute the relative current filename from the root of the current project, and contains a (-n overridable) file header, and includes any matching _header file_ found (in the current directory, or any compatible directory when [a.vim](http://code.google.com/p/lh-vim/source/browse/cpp/trunk/after/plugin/a.vim) is installed)

  * snippets for `main`, `for`, `for (int i`, `while`, `do...while`, `switch` and `case`, `if` ; NB: [lh-cpp](lhCpp.md) defines alternative [smart-snippets for these control-statements](lhCpp#Code_snippets.md), and several C++-only snippets in its [subversion repository](http://code.google.com/p/lh-vim/source/browse/#svn/cpp/trunk/after/template/cpp)
...
A few examples are better than a long speech, check the [documentation](http://code.google.com/p/lh-vim/source/browse/mu-template/trunk/doc/mu-template.txt) for more precisions.

Note: all the default template-files shipped with mu-template can be browsed from the [subversion repository](http://code.google.com/p/lh-vim/source/browse/mu-template/trunk/after/template/)
...
## Installation ##
  * Requirements: Vim 7.+, [lh-vim-lib](lhVimLib.md), [searchInRuntime](searchInRuntime.md), and my [bracketing-system](lhBrackets.md)
  * As a vimball: ~~[here (v2.3.0)](http://lh-vim.googlecode.com/files/mu-template-2.3.0.vba)~~
...
```
  * With [Vim Addon Manager](https://github.com/MarcWeber/vim-addon-manager) (and other plugin managers depending on [vim-pi](https://bitbucket.org/vimcommunity/vim-pi) like [NeoBundle](https://github.com/Shougo/neobundle.vim)) install [mu-template@lh](http://code.google.com/p/lh-vim/source/browse/mu-template/trunk/mu-template-addon-info.txt). This is the preferred method because of the various dependencies.
  * N.B.: installing [lh-cpp](lhCpp.md) or [lh-refactor](lhRefactor.md) with VAM will also install µtemplate.
